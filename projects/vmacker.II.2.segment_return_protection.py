...
def compile_operation(operation):
    ...
    if isinstance(VM[operation], str):
        return VM[operation]
    else:
        ASM['instance_label_index'] += 1
        return VM[operation](instance_label())

ASM['compiling'] = []
def compile_return():

    ASM['instance_label_index'] += 1
    if not ASM['compiling']:
        print(ASM['compiling'], ASM['functions'], ASM['calls'])
        raise SyntaxError( f"'return' used outside of function definition")
    ASM['compiling'].pop()
    return VM['return'](instance_label())
...
def compile_function_and_call(command, vm_name, n):
    ...
    if command == 'call':
        ...
        try:
            ...
            raise ValueError(
                    ...
                    f"must be a positive integer (not zero) "
                    f"below {segment_max['argument']}.\n"
                    f"At least one argument is needed to return the output.")
        # Make sure to have the function name in the list of function calls
        # and the minimum and maximum number of arguments ever called
        if asm_name not in ASM['calls']:
            ASM['calls'][asm_name] = (n, n)
        else:
            min_args, max_args = ASM['calls'][asm_name]
            ASM['calls'][asm_name] = (min(min_args, n), max(max_args, n))
        # Compile the code
        ASM['instance_label_index'] += 1
        # Return the compiled code
    ...
    if command == 'function':
        ...
        ASM['functions'][asm_name] = {'locals': n, 'args': 0}
        # Add it to the list of function bodies currently compiling
        ASM['compiling'].append(asm_name)
        print(ASM['functions'], ASM['compiling'])
        # Return the compiled code
...
def compile_segment(move, segment, i):
    ...
    # Make sure the function bodies use
    # only the amount of declared local variables
    # ASM['compiling'][-1] tells us which function we are compiling
    # ASM['functions'][function] tells us the declared number of locals
    if ASM['compiling']:
        function  = ASM['compiling'][-1]
        function  = ASM['functions'][function]
        if segment == 'local' and i >= function['locals']:
            raise ValueError(
                    f"'{line}' in body of function {ASM['compiling'][-1]}:"
                    f"index {i} out of range, declared {n_locals} local variables."
                    )
        # Keep track of how many arguments the function uses
        # To compare later with the function calls
        if segment == 'argument':
            function['args'] = max(function['args'], i)
    # Return the compiled code
...
def compile_vm_to_asm(vm_filename, asm_filename, debug=False):
    ...
    for function in ASM['calls']:
        try:
            # Get the number of arguments used in the function body
            # (if the function exists, othewise go to exception)
            used_args = ASM['functions'][function]['args'] + 1
            # Get the minimum number of arguments passed in the function call
            min_call_args, max_call_args = ASM['calls'][function]
            # Warn of argument overflow
            if used_args > min_call_args:
                import warnings
                warnings.warn(
                    f"function argument overflow:"
                    f"function {function} has a call with {min_call_args}, "
                    f"but the function body uses {used_args} arguments, "
                    f"overflowing into {used_args-used_args} stack entries",
                    SyntaxWarning)
            # Warn of unused arguments
            if used_args < max_call_args:
                import warnings
                warnings.warn(
                    f"unused function arguments:"
                    f"function {function} has a call with {max_call_args}, "
                    f"but the function body uses {used_args} arguments, "
                    f"thus leaving {max_call_args-used_args} unused arguments",
                    SyntaxWarning)
        except:
